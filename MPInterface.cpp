...
void ComputeVarianceMatrix(double** m1, double** m2, int Nd, int Nv, double** Gamma1, double** Gamma2) {
	for (int nb = 0; nb < Nd; nb++) {
		...
		for (int i = 0; i < Nv; i++) {
			Gamma1[nb][i] = (sum_1_m1 - (1 - m1[i][nb] * m1[i][nb])) / sqrt(Nv);
			Gamma2[nb][i] = (sum_1_m2 - (1 - m2[i][nb] * m2[i][nb])) / sqrt(Nv);
		}
	}
}
...
double ComputeMessageU(RBM* rbm, double** sigma_data, OrderParameter* op, int Nd, int nb, int i,int xi1, int xi2) {
	...
	double term3 = rbm->beta*rbm->beta*pow((sqrt(op->Gamma1[nb][i]) + sqrt(op->Gamma2[nb][i])), 2.) / 2.;
...
void ComputeExponentMatrix(int Nd, int Nv, double**** u, double**** expt) {
	for (int i = 0; i < Nv; i++) {
		...
		for (int na = 0; na < Nd; na++) {
			expt[i][na][0][0] = sum_u_0_0 - u[na][i][0][0];
			expt[i][na][0][1] = sum_u_0_1 - u[na][i][0][1];
			expt[i][na][1][0] = sum_u_1_0 - u[na][i][1][0];
			expt[i][na][1][1] = sum_u_1_1 - u[na][i][1][1];
		}
	}
}
